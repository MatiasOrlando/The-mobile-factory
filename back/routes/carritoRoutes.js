...
carritoRouter.post("/", async (req, res) => {
    ...
    try {
        ...
        const [item, created] = await Carrito.findOrCreate({ where: {customerId: 1, productId: 1}, defaults: {quantity: 1, price: product.price, productId: 1, customerId: 1}})
        if (!created) {
            const [index, updatedItem] = await Carrito.update({quantity: item.quantity + 8}, {where: {customerId: 1, productId: 1}, returning: true})
...
carritoRouter.delete("/", async (req, res) => {
    ...
    try {
        const item = await Carrito.findOne({where: {customerId: 1, productId: 1}})
...
carritoRouter.put("/", async (req, res) => {
    ...
    try {
        const [index, updatedItem] = await Carrito.update({quantity: 8}, {where: {customerId: 1, productId: 1}, returning: true})
        updatedItem.length ? res.send(updatedItem[0]) : res.status(404).send("ITEM NOT FOUND")
